...
import { Bed, Bath, Users, MapPin, Star, Search, CalendarIcon, ChevronRight, Maximize2 } from 'lucide-react'
...
export default function PropertiesPage() {
  ...
  const calculateTotal = (property: Property) => {
    ...
    const total = baseTotal + property.cleaningFee - discount + property.securityDepositAmount
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Buscar Propiedades</h1>
        <p className="text-muted-foreground">Encuentra tu próximo hogar temporal</p>
      </div>
            ...
            <CardFooter className="gap-2 pt-2">
              <Dialog>
                <DialogTrigger asChild>
                  <Button variant="outline" className="flex-1" onClick={() => setSelectedProperty(property)}>
                    Ver Detalles
                  </Button>
                </DialogTrigger>
                  ...
                  <div className="grid gap-4">
                    ...
                    <div className="grid gap-2 text-sm">
                      ...
                      <div className="mt-2 flex flex-wrap gap-4 text-muted-foreground">
                        <span className="flex items-center gap-1">
                          <Bed className="h-4 w-4" /> {property.bedrooms} habitaciones
                        </span>
                        <span className="flex items-center gap-1">
                          <Bath className="h-4 w-4" /> {property.bathrooms} baños
                        </span>
                        <span className="flex items-center gap-1">
                          <Users className="h-4 w-4" /> Máx. {property.maxGuests} huéspedes
                        </span>
                        <span className="flex items-center gap-1">
                          <Maximize2 className="h-4 w-4" /> {property.areaSqm}m²
                        </span>
                      ...
                      <div className="mt-4 rounded-lg bg-secondary p-4">
                        ...
                        <p className="mt-1 text-muted-foreground">{property.rules}</p>
                      </div>
                      <div className="mt-2 flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="text-2xl font-bold text-primary">${property.basePricePerNight}</p>
            ...
            <div className="grid gap-4">
                    ...
                    return (
                      <div className="mt-2 space-y-1 text-sm">
                        <div className="flex justify-between">
                          <span>${selectedProperty.basePricePerNight} x {calc.nights} noches</span>
                          <span>${calc.baseTotal.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Tarifa de limpieza</span>
                          <span>${selectedProperty.cleaningFee.toFixed(2)}</span>
                        </div>
                        {calc.discount > 0 && (
                        ...
                        <div className="flex justify-between">
                          ...
                          <span>${selectedProperty.securityDepositAmount.toFixed(2)}</span>
                        </div>
              ...
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                <p className="text-xs text-amber-800">
                  <strong>Política de Cancelación:</strong> Cancelación gratuita hasta 7 días antes del check-in. 
                  Entre 3-7 días: 50% de penalización. Menos de 3 días: sin reembolso.
                </p>
              </div>
            </div>
          )}
