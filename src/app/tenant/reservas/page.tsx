...
const errorTranslations: Record<string, string> = {
  "The property is not available for the requested extension dates.":
    "La propiedad no está disponible para las fechas solicitadas. Probablemente ya fue reservada por otra persona.",
  "The new check-out date must be after the current check-out date.":
    "La nueva fecha de salida debe ser posterior a la actual.",
  "Only RESERVED or ACTIVE reservations can be extended.":
    "Solo puedes extender reservas activas o en estado reservado.",
  "A valid payment method is required. 'PENDING' is not allowed for extensions.":
    "Debes seleccionar un método de pago válido.",
};

const translateApiError = (englishErrorMsg: string) => {
  if (!englishErrorMsg)
    return "Ha ocurrido un error inesperado al extender la reserva. Por favor, intenta de nuevo.";
...
export default function ReservationsPage() {
  ...
  const [stripeExtendPaymentId, setStripeExtendPaymentId] = useState<
    string | null
  >(null);
  ...
  const handleRateReservation = (reservation: ReservationResponse) => {
    router.push(`/tenant/calificaciones?reservationId=${reservation.id}`);
  };
  ...
  const handleViewDetails = async (reservation: ReservationResponse) => {
    ...
    try {
      const res = await reservationService.getLandlordReservationDetail(
        reservation.id,
      );
  ...
  const handleConfirmExtension = async () => {
    ...
    try {
      ...
      const res = await reservationService.extendReservation(
  ...
  const handleOpenCancelDialog = async (reservation: ReservationResponse) => {
    ...
    try {
      const response = await reservationService.previewCancellation(
        reservation.id,
      );
            ...
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="flex items-center gap-1.5 text-sm text-muted-foreground mt-0.5">
                  ...
                  {(reservation as any).propertyCity &&
                  (reservation as any).propertyDepartment
              ...
              <Badge
                className={cn(
                  "font-medium",
                  statusColors[reservation.reservationStatus] || "bg-gray-100",
                )}
              >
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {formatShortDateCard(reservation.checkInDate)}
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {formatShortDateCard(reservation.checkOutDate)}
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-2">
              ...
              <div className="space-y-5">
                <div className="rounded-xl bg-secondary/50 p-4">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {selectedDetail.property?.address ||
                      "Dirección específica disponible al hacer check-in"}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {selectedDetail.property?.city},{" "}
                    {selectedDetail.property?.department}
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-border p-4">
                  ...
                  <div className="space-y-2 text-sm text-muted-foreground">
                    <div className="flex justify-between">
                      ...
                      <span>
                        ${selectedDetail.baseTotal?.toFixed(2) || "0.00"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Tarifa de limpieza</span>
                      <span>
                        ${selectedDetail.cleaningFee?.toFixed(2) || "0.00"}
                      </span>
                    </div>
                    {selectedDetail.longStayDiscount > 0 && (
                      <div className="flex justify-between text-green-600">
                        ...
                        <span>
                          -${selectedDetail.longStayDiscount.toFixed(2)}
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between">
                      ...
                      <span>
                        $
                        {selectedDetail.securityDepositAmount?.toFixed(2) ||
                          "0.00"}
                      </span>
                    </div>
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        ...
                        >
                          ...
                          {newCheckoutDate
                            ? format(newCheckoutDate, "d 'de' MMMM yyyy", {
                                locale: es,
                              })
                            : "Selecciona fecha"}
                        </Button>
                    ...
                    <div className="rounded-xl bg-secondary/50 p-4">
                      ...
                      {(() => {
                        ...
                        const additionalNights = Math.round(
                          (newCheckout.getTime() - currentCheckout.getTime()) /
                            (1000 * 60 * 60 * 24),
                        );
                        const basePrice =
                          selectedDetail.property.basePricePerNight || 0;
                        ...
                        return (
                          <div className="space-y-2 text-sm text-muted-foreground">
                            <div className="flex justify-between">
                              <span>
                                ${basePrice.toFixed(2)} x {additionalNights}{" "}
                                {additionalNights === 1
                                  ? "noche adicional"
                                  : "noches adicionales"}
                              </span>
        ...
        <DialogContent className="max-w-sm sm:max-w-[425px]">
          <div className="flex flex-col items-center justify-center p-6 text-center space-y-4">
            ...
            <DialogDescription className="text-base text-center">
              Tu reserva para{" "}
              <strong>{selectedReservation?.propertyName}</strong> ha sido
              extendida con éxito. Las nuevas fechas y totales ya están
              reflejados en tu panel.
            </DialogDescription>
            <Button
                        ...
                        <div className="mt-2 space-y-1">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              $
                              {cancellationPreview.cancellationPenalty.toFixed(
                                2,
                              )}
                            </span>
                          </div>
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              $
                              {cancellationPreview.reservationRefundAmount.toFixed(
                                2,
                              )}
                            </span>
                          </div>
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              $
                              {cancellationPreview.cleaningFeeRefundAmount.toFixed(
                                2,
                              )}
                            </span>
                          </div>
